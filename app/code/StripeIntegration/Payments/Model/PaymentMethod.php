    ...
    public function cancel(InfoInterface $payment, $amount = null)
    {
        ...
        try {
            ...
            if (!$this->checkIfWeCanRefundMore($refundedAmount, $remainingAmount, $requestedAmount, $order, $currency))
                return $this->setRefundedAmount($refundedAmount, $requestedAmount, $currency, $order);
            ...
            // We are calling it one last time in case an order comment/warning needs to be added
            if (!$this->checkIfWeCanRefundMore($refundedAmount, $remainingAmount, $requestedAmount, $order, $currency))
                return $this->setRefundedAmount($refundedAmount, $requestedAmount, $currency, $order);
        }
        catch (\Exception $e)
